...
def traing_GPModel(num_epochs = 1000, 
                   lr0 = 0.01, 
                   validate = False, 
                   ylabel = 'yy', 
                   best_loss = 9999, 
    ...
    if validate == True:
        ...
        ax.set_yscale('log')
        ax.set_xscale('log')
        ax.set_ylabel('Validating error')
        ax.set_xlabel('Training epoch')
        fig.savefig(savedr + ylabel + '_validation.png')
    ...
    for i in range(num_epochs):
        ...
        if ((i%interval)==0 and (loss.item() < loss_cut) ):
            ...
            if validate == True:
                ...
                with torch.no_grad(), gpytorch.settings.fast_pred_var():
                    observed_pred = likelihood(model(vali_x))
                    mean = observed_pred.mean
                    dyy = (mean - vali_y)
                    dyy = dyy.cpu()
                    dyy = dyy*(dfref[yy].std())
                    dd = np.percentile(np.abs(dyy), [68,95,99.8,100])
                    vali_error = np.sum(np.percentile(np.abs(dyy), [68,95,100.0]))
                    ax.scatter([i,i], [dd[0], dd[0]], c = 'k')
                    ax.scatter([i,i], [dd[1], dd[1]], c = 'b')
                    ax.scatter([i,i], [dd[2], dd[2]], c = 'r')
                    fig.savefig(savedr + ylabel + '_vali_vs_epoch.png')    
                model.train()
                likelihood.train()
                
                if ((best_loss - vali_error) >= 1.0e-3):
                    best_loss = vali_error
                    dds0 = dd
        ...
        if ((i%50)==0): 
            print(i,loss.item(), best_loss, t, tr, lr0, model.likelihood.noise.item())
            print('so far best vali errors:', dds0)
 ...
 # for adjusting learning rate
        ...
        stop = np.min([300, int(500/interval)])
        
        if (t > stop) or (lr0 < 1.0e-4):
            print(yy + 'no improvements and terminate')
...
os.system(u'ls ../../gp_data_v3_scale')
# In[6]:

dr = 'subsections_GP_NNElu_Mat32_3sections/'
os.system("mkdir subsections_GP_NNElu_Mat32_3sections/")

# set up an reference data set
df0 = pd.read_csv('../../gp_data_v3_scale/training.csv',index_col = 0)
dfref = df0.sample(n = 10000, weights= 'hrgradient', random_state = 999)
dfref.to_csv(dr + 'reference.csv')
...
for i in range(nsections):
    ...
    E0 = round( i*section, 3)
    E1 = round( (i+1)*section, 3)
    EEPrange = [E0, E1]
    print('Now train the data within EEP = ', EEPrange)
    
    if E0<0.0: continue
    
    label = 'EEP' + str(E0) + '-' + str(E1)
    savedr =  dr + label + '/'
    command = 'mkdir ' + savedr
    os.system(command)
    ...
    df0 = pd.read_csv('../../gp_data_v3_scale/training.csv',index_col = 0)
    nuse = ((df0.evol_scale >=EEPrange[0]) 
        & (df0.evol_scale <= EEPrange[1]) )
    df = df0[nuse]
    print('The total data length is ', len(df))
    print('so the sampleing rate is ', ntraining/len(df)) 
    
    nt = np.min([ntraining, len(df)])
    
    train = df.sample(n = nt, weights= 'hrgradient', random_state = 999)
    train.index = range(len(train))
    train.to_csv(savedr + label + '_training.csv')
    
    
    vv0 = pd.read_csv('../../gp_data_v3_scale/validation_5d.csv',index_col = 0)
    nuse = ((vv0.evol_scale >=EEPrange[0]) 
        & (vv0.evol_scale <= EEPrange[1]) )
    
    nvali = np.min([1000, len(vv0[nuse])])
    
    validation = vv0[nuse].sample(n = nvali, random_state = 987)
    validation.index = range(len(validation))
    
    print('sampleing rate is ', nvali/len(vv0[nuse]))
    
    # normalized data for training
    gdf = normalize_a_df(gdf = train, df0 = dfref, xcolumns = xcolumns, ycolumns = ycolumns)
    vali = normalize_a_df(gdf = validation, df0 = dfref, xcolumns = xcolumns, ycolumns = ycolumns)
    
    # all outputs
    ycolumns = ["effective_T","log_g","radius","delta_nu_fit", "star_feh", "star_age"]
    # here we define the point where ealry stop trigered
    loss_cuts = [-2.7, -2.7, -2.7, -2.7, -2.5, -2.7]
    
    for ii in range(len(ycolumns)):
        yy = ycolumns[ii]
        loss_cut = loss_cuts[ii]
        ...
        # define training and validating set
        train_x = torch.tensor((gdf[xcolumns].values)).to(dtype=torch.float32) 
        train_y = torch.tensor((gdf[yy].values)).to(dtype=torch.float32)
        vali_x = torch.tensor((vali[xcolumns].values)).to(dtype=torch.float32) 
        vali_y = torch.tensor((vali[yy].values)).to(dtype=torch.float32)
        ...
        torch.save(train_x, savedr + ylabel + '_train_x.pt')
        torch.save(train_y, savedr + ylabel + '_train_y.pt')
        ...
        best_loss1 = traing_GPModel(num_epochs = num_epochs, 
                                   lr0 = lr0, 
                                   validate = True, 
                                   ylabel = ylabel, 
                                   best_loss = 9999,
